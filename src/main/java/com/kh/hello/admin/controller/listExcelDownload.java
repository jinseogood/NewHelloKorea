...
public class listExcelDownload extends AbstractXlsxView {
	...
	protected void buildExcelDocument(Map<String, Object> modelMap, Workbook workbook, HttpServletRequest request,
        ...
        // 헤더 설정
        row = worksheet.createRow(0);
        row.createCell(0).setCellValue("업체ID");
        ...
        for(Deposit d : listExcel) {
            row = worksheet.createRow(rowIndex);
            row.createCell(0).setCellValue(d.getcId());
            row.createCell(1).setCellValue(d.getcName());
        ...
        worksheet.addMergedRegion(
                new CellRangeAddress(listExcel.size() + 1, listExcel.size() + 1, 0, 5));
